...
public class CrossWordsGameGrid_old
{
    
    public List<GridWord_old> Words { get; private set; }
    public event Action<GridWord_old> OnValidateAllWorlds;
    public event Action<GridWord_old> OnAddWord; 

    public CrossWordsGameGrid_old( List<GridWord_old> words)
    {
        Words = new List<GridWord_old>();
    ...
    private Dictionary<Vector2, char> GetAnyWordListToGridValues(List<GridWord_old> words)
    {
    ...
    public GridWord_old? GetWordAtLocation(Vector2 location)
    {
    ...
    public void AddWord(GridWord_old word)
    {
        Words.Add(word);
    ...
    private void OnWordValidateCallback(GridWord_old word)
    {
    ...
    /// <returns></returns>
    public List<GridWord_old> GetAllWordAtLocation(Vector2 location)
    {
        var characterPlacements = GetWordsToGridValues();
        ...
        List<GridWord_old> result = new List<GridWord_old>();
        bool isRow = characterPlacements.Keys.Contains(new Vector2(location.x + 1, location.y)) || 
                     characterPlacements.Keys.Contains(new Vector2(location.x - 1, location.y));
        bool isColumn = characterPlacements.Keys.Contains(new Vector2(location.x , location.y + 1)) || 
                        characterPlacements.Keys.Contains(new Vector2(location.x , location.y - 1));
    ...
    public GridWord_old? GetWordWithStartPosition(Vector2 startPosition, bool isRow)
    {
        foreach (var word in Words)
        {
    ...
    private KeyValuePair<Vector2, Vector2> GetMinAndMaxPositionCharacterPlacementOfWordList(List<GridWord_old> gridWords)
    {
    ...
    public Vector2 GetMiddleWordList(List<GridWord_old> gridWords)
    {
    ...
    public List<GridWord_old> GetAllNonValidatedWords()
    {
        List<GridWord_old> result = new List<GridWord_old>();
        foreach (var word in Words)
        {
    ...
    public List<Vector2> RevealLetterInAllWords(char letter)
    {
        ...
        foreach (GridWord_old word in Words)
        {
